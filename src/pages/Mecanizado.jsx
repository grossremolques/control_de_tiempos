...
export default function Mecanizado() {
  const { mecanizado, getMecanizado, updateMecanizado, setResponseGeneral } =
    useGlobal();
  ...
  useEffect(() => {
    if (mecanizado && mecanizado.length > 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { status, error } = await updateMecanizado(modalData.id, formData);
      if (error) {
